...
一把百炼 Key 覆盖改字、看图拆单、局部重画：

| 周 | 作用 | `.env` 项 | 现在的模型名 |
|---|---|---|---|
| 2 | 改字 | `DASHSCOPE_REWRITE_MODEL` | `qwen3.6-flash` |
| 3 | 看图拆单 | `DASHSCOPE_PLANNER_MODEL` | `qwen3-vl-plus` |
| 2 | 按 mask 局部重画 | `DASHSCOPE_INPAINT_MODEL` | `wanx2.1-imageedit` |

1. **百炼**： [API-Key 管理](https://bailian.console.aliyun.com/?tab=model#/api-key) 创建密钥；模型广场开通 **文本生成 `qwen3.6-flash`**、**视觉理解 `qwen3-vl-plus`**，以及 **图像编辑 `wanx2.1-imageedit`**（华北 2（北京））。旧名 `qwen-plus` / `qwen-vl-max` 已下线。
2. `copy .env.example .env`，填 `DASHSCOPE_API_KEY`、`DASHSCOPE_BASE_URL`（控制台「OpenAI 兼容」那一行）。不必再填 fal。
3. 关掉再开 `npm run dev`，点 **检查接口**。应看到百炼、万相已接入。
4. 真要改写/改图时：`.env` 把 `MARKSET_ALLOW_MODEL_CALLS` 改为 `1`，重启，再勾页面上的「允许调用云端模型」。
5. **松手贴轮廓**已关掉：选区就是鼠标圈的范围，不再调用云端 SAM。

本机转发：`POST /api/rewrite`、`/api/plan`、`/api/inpaint`。无双开开关注则一律 403，不会打到百炼。
